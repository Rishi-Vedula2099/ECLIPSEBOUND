...
# RPG Progression & Inventory Signals
signal level_up(new_level: int, stat_points: int, skill_points: int)
signal world_level_changed(new_world_level: int)
signal boss_mastery_updated(boss_id: String, new_mastery: int)
signal skill_unlocked(branch: String, skill_id: String)
signal weapon_equipped(weapon: Resource)
signal weapon_mastery_leveled(weapon_class: String, new_level: int)
signal item_looted(item: Resource)
signal item_salvaged(item: Resource, yield_dict: Dictionary)
